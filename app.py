...
def generate_report(report_type):
    ...
    p.drawCentredString(width /2.0, height - 90, "Tel No.: 941-3833         Cell No.: 0917 732 4523")
    if report_type == 'appointments':
...
def generate_billing(patient_id):
    ...
    # Define cost mapping for conditions
    condition_costs = {
        'X': 2533,
        'XO': 3045,
        'Am': 213,
    ...
    # Convert diagnoses to list of dictionaries
    diagnoses = [dict(diagnosis) for diagnosis in diagnoses]

    total_cost = 0
    for diagnosis in diagnoses:
        cost = condition_costs.get(diagnosis['condition_code'], 0)
        diagnosis['cost'] = cost
        total_cost += cost

    # Fetch items used details from the item_billing table
    items_used = conn.execute('''
        SELECT * FROM item_billing WHERE patient_id = ?
    ''', (patient_id,)).fetchall()

    # Convert items_used to list of dictionaries
    items_used = [dict(item) for item in items_used]
